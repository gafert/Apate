...
export class MemoryComponent implements OnInit, OnDestroy {
  ...
  constructor(private SimLibInterfaceService: SimLibInterfaceService) {
    SimLibInterfaceService.bindings.mem_addr__subject
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe((value) => {
      this.mem_addr = value;
    });
    SimLibInterfaceService.bindings.mem_rdata__subject
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe((value) => {
      this.mem_rdata = value;
    });
    SimLibInterfaceService.bindings.mem_wdata__subject
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe((value) => {
      this.mem_wdata = value;
    });
    SimLibInterfaceService.bindings.mem_valid__subject
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe((value) => {
      this.mem_valid = value;
    });
    SimLibInterfaceService.bindings.memory__subject
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe((value) => {
      this.memory = value;
    });
  }
