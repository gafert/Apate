...
export class CompileComponent implements OnDestroy, AfterViewInit, OnInit {
	...
	public objdumpFlags =
		'--section .text.init --section .text --section .data --full-contents --disassemble --syms --source -z';
	/** Not changed by the user */
	...
	clickFile(event, file: string) {
		const txtFiles = ['.c', '.txt', '.s', '.c', '.cc', '.cpp', '.h', '.hpp', '.hh', '.js', '.ts', '.ld'];
	...
	fileContentEdited(code) {
		if (this.fileIsSavable && this.activeFile && this.folderPath) {
			fs.writeFile(path.join(this.folderPath, this.activeFile), code, (err) => (err ? console.log(err) : null));
